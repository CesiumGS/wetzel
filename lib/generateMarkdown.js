...
function propertySpecs(schema) {
    return [
        {
            label: "Properties",
            properties: schema.properties,
            formatName: function(n) { return n },
        },
        {
            label: "Pattern Properties",
            properties: schema.patternProperties,
            formatName: function(n) { return style.typeValue(new RegExp(n)) },
        },
    ]
}
...
function createPropertiesSummary(schema, knownTypes, autoLink) {
    ...
    for (const propType of propertySpecs(schema)) {
        var properties = propType.properties

        if (properties !== undefined && Object.keys(properties).length > 0) {
            md += style.beginTable(style.typeValue(schema.title) + ' ' + propType.label, ['   ', 'Type', 'Description', 'Required']);
            ...
            for (var name in properties) {
                if (properties.hasOwnProperty(name)) {
                    ...
                    md += style.addTableRow([
                        style.propertyNameSummary(propType.formatName(name)),
