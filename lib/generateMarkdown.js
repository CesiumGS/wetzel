        ...
        if (properties.hasOwnProperty(name)) {
            ...
            if (defined(items)) {
                ...
                if (defined(items.minimum) && defined(items.maximum)) {
                    md += '   * Each element in the array must be ' + minString + ' ' + items.minimum + ' and ' + maxString + ' ' + items.maximum + '.\n';
                } else if (defined(items.minimum)) {
                    md += '   * Each element in the array must be ' + minString + ' ' + items.minimum + '.\n';
                } else if (defined(items.maximum)) {
                    md += '   * Each element in the array must be ' + maxString + ' ' + items.maximum + '.\n';
                }

                if (defined(items.minLength) && defined(items.maxLength)) {
                    md += '   * Each element in the array must have length between ' + items.minLength + ' and ' + items.maxLength + '.\n';
                } else if (defined(items.minLength)) {
                    md += '   * Each element in the array must have length greater than or equal to ' + items.minLength + '.\n';
                } else if (defined(items.maxLength)) {
                    md += '   * Each element in the array must have length less than or equal to ' + items.maxLength + '.\n';
                }
            }
            ...
            if (defined(minimum)) {
                ...
                md += '* ' + stylePropertyDetails('Minimum') + ':' + (exclusiveMinimum ? ' > ' : ' >= ') + minimum + '\n';
            }

            var maximum = property['maximum'];
            if (defined(maximum)) {
                var exclusiveMaximum = (defined(property.exclusiveMaximum) && property.exclusiveMaximum);
                md += '* ' + stylePropertyDetails('Maximum') + ':' + (exclusiveMaximum ? ' < ' : ' <= ') + maximum + '\n';
            }
