...
var styleAsciiDoctor = require('./styleAsciiDoctor');
var styleMarkdown = require('./styleMarkdown');
var style;
...
function generateMarkdown(options) {
    ...
    style = (defined(options.styleMode) && options.styleMode === 'AsciiDoctor') ? styleAsciiDoctor : styleMarkdown;
...
function createPropertiesSummary(schema, knownTypes, autoLink) {
    ...
    if (schema.properties !== undefined && Object.keys(schema.properties).length > 0) {
        md += style.beginTable(schema.title + ' Properties', ['   ', 'Type', 'Description', 'Required']);
