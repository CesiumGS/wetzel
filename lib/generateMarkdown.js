...
function generateMarkdown(options) {
    ...
    if (schemaType === 'object') {
        ...
        // Render table with summary of each pattern property
        md += createPatternPropertiesSummary(schema, options.suppressWarnings);
        ...
        // Render section for each property & definition
        md += createPropertiesDetails(schema, title, options.headerLevel + 1, options.suppressWarnings);
        md += createPropertiesDetails(schema, title, options.headerLevel + 1, options.suppressWarnings, 'definitions');
    }
...
function createPatternPropertiesSummary(schema, suppressWarnings) {
    var md = '';
    md += style.propertiesSummary('Pattern Properties') + '\n\n';
    md += '|   |Type|Description|Required|\n';
    md += '|---|----|-----------|--------|\n';

    var properties = schema.patternProperties;
    for (var name in properties) {
        if (properties.hasOwnProperty(name)) {
            var property = properties[name];
            var summary = getPropertySummary(property, suppressWarnings);
...
var alreadyDetailed = []
function createPropertiesDetails(schema, title, headerLevel, suppressWarnings, section) {
    var headerMd = style.getHeaderMarkdown(headerLevel);
    var md = '';
    if (section === undefined) section = 'properties'
    var properties = schema[section];
    for (var name in properties) {
        if (properties.hasOwnProperty(name) && alreadyDetailed.indexOf(name) === -1) {
            alreadyDetailed.push(name);
            var property = properties[name];
