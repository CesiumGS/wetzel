...
function generateMarkdown(options) {
    ...
    if (schemaType === 'object' || schema.hasOwnProperty('properties') || schema.hasOwnProperty('patternProperties')) {
        // Render table with summary of each property
        md += createPropertiesSummary(schema, options.suppressWarnings);
        md += createPropertiesSummary(schema, options.suppressWarnings, 'pattern');
...
function createPropertiesSummary(schema, suppressWarnings, ptype) {
    var md = '';
    var title;
    var properties;
    if (ptype === 'pattern') {
        if (!schema.hasOwnProperty('patternProperties')) return '';
    ...
    md += style.propertiesSummary(title) + '\n\n';
