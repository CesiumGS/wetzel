...
* Appends children elements to the table of contents
* @param  {object} orderedTypes The ordered collection of types for the TOC.
* @param  {string} parent_type  A string that contains the type of the parent object.
* @param  {int} depth           The number of indentation levels that should be applied.
* @return {string} The markdown for the table of contents.
*/
function getRecursiveTOC(orderedTypes, parent_type, depth) {
    var md = '';
    for (var i = 0; i < orderedTypes[parent_type].children.length; i++) {
        var currentType = orderedTypes[parent_type].children[i];
        if (orderedTypes[currentType].parents.length == 1) {
            md += style.bulletItem(style.linkType(style.typeValue(currentType), currentType, enums.autoLinkOption.codeQuoteOnly), depth);
