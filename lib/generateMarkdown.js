...
function getTableOfContentsMarkdown(schema, orderedTypes, headerLevel) {
    ...
    for (var type in orderedTypes) {
        ...
        var isRelativeReference = defined(orderedTypes[type].fileName) && orderedTypes[type].fileName.indexOf('#') > -1;
        if (type === schema.title || orderedTypes[type].parents.length > 1 || orderedTypes[type].parents.indexOf(schema.title) !== -1 || isRelativeReference) {
...
function getPropertySummary(property, knownTypes, autoLink) {
    ...
    if (Array.isArray(type)) {
        ...
        formattedType = formattedType.slice(0, -2);
    } else if (type === 'array') {
        formattedType = formatArrayType(type, property, autoLink);
    } else {
        formattedType = style.typeValue(type);
    }
