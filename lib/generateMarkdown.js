...
function getSchemaMarkdown(schema, fileName, headerLevel, suppressWarnings, schemaRelativeBasePath, knownTypes, autoLink, embedMode) {
    ...
    if (schemaType === 'object') {
        ...
        md += getAdditionalProperties(schema, autoLink, 'Type of additional properties')
            + getPropertyNames(schema, summary, title, knownTypes, 0, autoLink)
...
function getPropertyNames(schema, summary, title, knownTypes, depth, autoLink) {
    if (!defined(schema.propertyNames)) return ''
    var names = schema.propertyNames
    names.type = 'string' // property names are always strings, so usually omitted.
    return style.bulletItem(style.propertyDetails('Property names') + ':', depth)
