...
function replaceRef(schema, fileName, searchPaths, ignorableTypes, schemaReferences, schemaRepository, parentTitle, root) {
    ...
    if (defined(ref)) {
        ...
        if (defined(refEntry)) {
            ...
            // TODO The 'ref' may contain a fragment. The 'ignorableTypes' contains file names.
            // The assumption that "a type" and "a file" are the same concept is tempting,
            // but wrong when types are defined locally (in 'definitions'). This has to be
            // sorted out, but might require larger refactorings...
            ...
            replaceRef(refSchema, refFileName, searchPaths, ignorableTypes, schemaReferences, schemaRepository, schema.title === undefined ? parentTitle : schema.title, root);
        }
        mergeProperties(schema, refSchema);
        delete schema.$ref;
    }
